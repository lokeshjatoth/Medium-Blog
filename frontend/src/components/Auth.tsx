...
const Auth = ({ type }: AuthProps) => {
    ...
    const submitHandler = async (event: React.FormEvent) => {
        ...
        try {
            ...
            setIsLoading(false); // Reset loading status after request
        }
    };
                ...
                <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                    <div>
                        ...
                        <p className="text-md text-gray-500 dark:text-gray-400 text-center mt-2">
                            ...
                            <Link to={type === "signup" ? "/signin" : "/signup"} className="font-medium text-primary-600 hover:underline dark:text-primary-500 underline">
                    ...
                    <form className="space-y-4 md:space-y-6" onSubmit={submitHandler}>
                        ...
                        <button type="submit" className="text-white bg-gray-800 hover:bg-gray-900 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 w-full hover:cursor-pointer" disabled={isLoading}>
                            {isLoading ? <Spinner /> : (type === "signup" ? "Sign up" : "Sign In")}
                        </button>
                    </form>
                </div>
            </div>
